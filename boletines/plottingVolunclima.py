...
def graficarPercepcionSequiaMensual(yyyy,mm,isoCty, dfStations, dfValues):
	...
	if isoCty=='EC':
		...
		plt.suptitle('Percepción de sequía en Bolivia ('+strMonth+')')
		lon_leftup=-69.625
		lat_leftup=-9.625
		lon_rightdown=-57.375
		lat_rightdown=-22.875
		map = Basemap(anchor='W', projection='merc',llcrnrlat=lat_rightdown,urcrnrlat=lat_leftup,llcrnrlon=lon_leftup,urcrnrlon=lon_rightdown,resolution='i')#lat_ts=7,resolution='i'
		map.drawparallels(np.arange(-10, -22, 1),labels=[True,False,False,False])
		map.drawmeridians(np.arange(-70,-9, 1),labels=[False,False,False,True])
	elif isoCty == 'CO':
...
def graficarPrecipitacionDiariaRed(datDate,isoCty,dfStations,dfValues,fltTrhld):
	...
	if isoCty=='EC':
		...
		lat_rightdown=-22.875
		map = Basemap(anchor='W', projection='merc',llcrnrlat=lat_rightdown,urcrnrlat=lat_leftup,llcrnrlon=lon_leftup,urcrnrlon=lon_rightdown,resolution='i')#lat_ts=7,resolution='i'
		map.drawparallels(np.arange(-10, -22, 1),linewidth=0,dashes=(None,None),labels=[True,False,False,False])
		map.drawmeridians(np.arange(-70,-9, 1),linewidth=0,dashes=(None,None),labels=[False,False,False,True])
	elif isoCty=='CO':
		strCty = "Colombia"
