...
def generarTemperaturaEstacionalSuramerica(nombre_archivo='temperatura.nc',region=[ 30, -120, -60,-30],mes_str="",year_str=""):
    try:
        ...
        if not os.path.exists(dirName):
            os.makedirs(dirName)
        generarDataset('seasonal-monthly-single-levels',nombre_archivo,year_str,mes_str,'2m_temperature','ensemble_mean',['1', '2', '3'],region)
...
def generarPrecipitacionEstacionalSuramerica(nombre_archivo='precipitacion.nc',region=[ 30, -120, -60,-30],mes_str="",year_str=""):
    try:
        ...
        if not os.path.exists(dirName):
            os.makedirs(dirName)
        generarDataset('seasonal-monthly-single-levels',nombre_archivo,year_str,mes_str,'total_precipitation','ensemble_mean',['1', '2', '3'],region)
...
def generarAnomaliaTemperaturaEstacionalSuramerica(nombre_archivo='temperatura_anomalia.nc',region=[ 30, -120, -60,-30],mes_str="",year_str=""):
    try:
        ...
        if not os.path.exists(dirName):
            os.makedirs(dirName)    
        generarDataset('seasonal-postprocessed-single-levels',nombre_archivo,year_str,mes_str,'2m_temperature_anomaly','ensemble_mean',['1', '2', '3'],region)
...
def generarAnomaliaPrecipitacionAnomaliaEstacionalSuramerica(nombre_archivo='precipitacion_anomalia.nc',region=[ 30, -120, -60,-30],mes_str="",year_str=""):
    try:
        ...
        if not os.path.exists(dirName):
            os.makedirs(dirName)
        generarDataset('seasonal-postprocessed-single-levels',nombre_archivo,year_str,mes_str,'total_precipitation_anomalous_rate_of_accumulation','ensemble_mean',['1', '2', '3'],region)
